...
they check the places they think he was most likely to
visit.
...
def solve2(list1:list, list2:list):
    ...
    for val in list1:
        if val in val_dict:
            sim_score += val_dict[val]
            continue
        low = bisect.bisect_left(list2, val)
        high = bisect.bisect_right(list2, val)
        if low == high:
            val_dict[val] = 0
            continue
        temp_val = (high - low) * val
        sim_score += temp_val
        val_dict[val] = temp_val
    return sim_score
    
    
# print(solve2([3,4,2,1,3,3], [4,3,5,3,9,3]))
